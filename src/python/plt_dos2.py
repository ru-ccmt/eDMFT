...
if __name__=='__main__':
    ...
    parser.add_argument('-r', default=None,type=str, help='list of which imp2lattice to remove')
    ...
    #    #iatom,L = inl.cix[icix][:2]
    #    sigind = inl.siginds[icix]
    #    found=False
    #    for ic in unique_dos_deg:
    #        if len(inl.siginds[ic])==len(sigind) and allclose(inl.siginds[ic],sigind):
    #            unique_dos_deg[ic] += 1
    #            found=True
    #            break
    #    if not found:
    #        unique_dos_deg[icix] = 1
    #
    #unique_dos_deg = {**{0:1}, **unique_dos_deg}
    #print('unique_dos with degeneracy=', unique_dos_deg)
    #print('degs[icix]=', degs[icix])

    unique_dos={}
    for icix in _lcols:
        found=False
        cl = _lcols[icix]
        for ic in unique_dos:
            if len(_lcols[ic])==len(cl) and allclose(_lcols[ic],cl):
                unique_dos[ic] += 1
                found=True
                break
        if not found:
    ...
    unique_dos = {**{0:1}, **unique_dos}
    print('unique_dos=', unique_dos)
    ...
    DOS = dat[1:,:]
    il=0
    for icix in unique_dos:
        leg1 = leg[icix]
        if icix>0:
            iatm,L = inl.cix[icix][0][0:2]
            atm = atom_names[iatm-1]
            leg1 = atm+',L='+str(L)
        linei,= axs.plot(om,DOS[icix,:]*unique_dos[icix], 'C'+str(il%10), label=leg1)
        lines.append(linei)
        il+=1
        ...
        for icix in imp2latt[ii]:
            if icix in lcols and not foundOneYet:
                ...
                for i,j in enumerate(cls):
                    linei,= axs.plot(w,dos[i], 'C'+str(il%10)+'-', lw=1,label=atm+'[$'+inl.legends[icix][j]+'$]')
                    lines.append(linei)
                    il+=1
            if lcols_dn is not None and icix in lcols_dn and not foundOneYetdn:
                ...
                for i,j in enumerate(cls):
                    linei,=axs.plot(w,-Glat[i].imag/pi, 'C'+str(il%10)+'-',lw=1, label='glat['+str(icix)+'dn,$'+inl.legends[icix][j]+'$]')
                    lines.append(linei)
                    il+=1
