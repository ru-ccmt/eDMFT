...
!!!----------------------------------------------------------------------!!!
  ...
  COMPLEX*16 :: ctmp, zsum               ! Temporary variable 
  !COMPLEX*16 :: tmp(ndim,ndim)  ! Transformed Hamiltonian
  COMPLEX*16 :: scaler(ndim), xdot     ! Array of normalization parameters
  COMPLEX*16 :: cworkvec(8*ndim) ! Work array for zgeev
  REAL*8     :: rworkvec(8*ndim) ! Work array for zgeev
  INTEGER    :: idxarr(ndim)
  COMPLEX*16,ALLOCATABLE :: dotlr(:,:)
  ...
  ALLOCATE( dotlr(ndim,ndim) )
  dotlr(:,:)=0.0

  !$OMP PARALLEL DO SHARED(dotlr,zek) PRIVATE(p,xdot,i) SCHEDULE(STATIC) 
  DO q=1,ndim
     DO p=1,ndim
        IF (abs(zek(p)-zek(q)).LT.smalleps) then
           xdot = (0.0D0,0.0D0)
           DO i=1,ndim
              xdot = xdot + evl(p,i)*evr(i,q)
           ENDDO
           dotlr(p,q) = xdot
        END IF
     ENDDO
  ...
  DO q=1,ndim
     DO p=1,q-1
        IF (abs(zek(p)-zek(q)).LT.smalleps .AND. abs(dotlr(p,q)).GT.smalleps) THEN
           evr(:,q) = evr(:,q) - dotlr(p,q)/dotlr(p,p) * evr(:,p)
        ENDIF
     ENDDO
     DO p=1,q-1
        IF (abs(zek(p)-zek(q)).LT.smalleps .AND. abs(dotlr(q,p)).GT.smalleps) THEN
           evl(q,:) = evl(q,:) - dotlr(q,p)/dotlr(p,p) * evl(p,:)
        ENDIF
     ENDDO
  ENDDO
  DEALLOCATE( dotlr )
...
SUBROUTINE eigvals0(ham, zek, ndim)
!!!----------------------------------------------------------------------!!!
!!! Eigenvalues only, no eigenvectors                                    !!!
!!! This routine solves the generalized right/left eigenvalue problems   !!!
!!!       H.Ev^{R} = E Ev^{R}      and    Ev^{L}H = E Ev^{L}             !!!
