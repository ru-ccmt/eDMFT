...
if __name__ == '__main__':
    ...
    smallShift  = float(next(fin).split()[0]) # smallShift
    print 'smallShift=', smallShift
    exec(next(fin))  # CheckC4Symmetry
    exec(next(fin))  # EnforceC4Symmetry
    print 'CheckC4Symmetry=', CheckC4Symmetry
    print 'EnforceC4Symmetry=', EnforceC4Symmetry
    if CheckC4Symmetry or EnforceC4Symmetry:
        exec(next(fin)) # symm=[2,3]
        exec(next(fin)) # rest=[0,1,4]
        exec(next(fin)) # imk=[0,1,1]
        print 'symm=', symm
        print 'rest=', rest
        print 'imk=', imk
